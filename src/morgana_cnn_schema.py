...
class MorganaCNNSchema(CNNSchema):
    ...
    def __init__(self, num_classes, vocab_size, embedding_size, filter_sizes, num_filters,
        ...
        dense_layers = tf.concat([words_dl, chars_dl], 1)

        # dropout_layer_output = self.create_dropout_layer(convolution_layer_output,dropout_prob=self.dropout_keep_prob)
        scores, predictions, l2_loss = self.create_output_layer(dense_layers, num_classes * 6,
                                                                num_classes,
                                                                l2_loss=l2_loss)
    ...
    def map_batch(tweets):
        ...
        for t_id in tweets:
            ...
            embed_tokens = MorganaCNNSchema.MAPPER.map_features(tokens, t_id)
