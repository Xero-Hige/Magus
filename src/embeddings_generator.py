...
def generate_word_embeddings():
    sentences = TrainSentenceGenerator(["../database"], WordTokenizer)
...
def generate_raw_chars_embeddings():
    sentences = TrainSentenceGenerator(["../database"], RawCharTokenizer)
    model = gensim.models.Word2Vec(sentences, min_count=5, size=150)
    word_vector = model.wv
    word_vector.save('./rCharsEmbeddings.mdl')


def generate_chars_embeddings():
    sentences = TrainSentenceGenerator(["../database"], CharTokenizer)
    model = gensim.models.Word2Vec(sentences, min_count=5, size=150)
    word_vector = model.wv
