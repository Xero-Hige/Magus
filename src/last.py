...
class MorganaCNNSchema(CNNSchema):
    ...
    def __init__(self, num_classes, vocab_size, embedding_size, filter_sizes, num_filters,
        ...
        self.char_loss = None
        self.char_accuracy = None
        self.rchar_loss = None
        self.rchar_accuracy = None
        self.scores = None
        ...
        with tf.name_scope("words_stream"):
            words_dl, hidden_words_dl = self.create_first_combs(self.input_x_words, embedding_size, filter_sizes,
                                                                l2_loss, num_filters, MAX_WORDS, "words_stream",
        ...
        with tf.name_scope("chars_stream"):
            chars_dl, hidden_chars_dl = self.create_first_combs(self.input_x_chars, embedding_size, filter_sizes,
                                                                l2_loss, num_filters, MAX_CHARS, "chars_stream",
                                                                output_size=output_layers_size,
                                                                hidden_layer_size=hidden_layer_size)
        ...
        with tf.name_scope("rchar_stream"):
            r_chars_dl, hidden_rchar_dl = self.create_first_combs(self.input_x_rchars, embedding_size, filter_sizes,
                                                                  l2_loss, num_filters, MAX_CHARS, "rchar_stream",
                                                                  output_size=output_layers_size,
                                                                  hidden_layer_size=hidden_layer_size)
        ...
        with tf.name_scope("global"):
            dense_layers = tf.concat([hidden_words_dl, hidden_chars_dl, hidden_rchar_dl], 1)

            global_dense_layer = self.create_dense_layer(dense_layers, hidden_layer_size * 3, hidden_layer_size,
                                                         name="{}/concat_dense".format("global"), l2_loss=l2_loss)

            dropout = self.create_dropout_layer(global_dense_layer, dropout_prob=self.dropout_keep_prob)

            scores, predictions = self.create_output_layer(dropout, hidden_layer_size, num_classes, l2_loss=l2_loss)
    ...
    def create_first_combs(self,
        ...
        dense_layer = self.create_dense_layer(convolution_layer_output, output_channels, hidden_layer_size,
                                              name="{}/hidden_dense".format(name), l2_loss=l2_loss)
        ...
        redux_dense_layer = self.create_dense_layer(dropout, hidden_layer_size, output_size,
                                                    name="{}/redux_dense".format(name), l2_loss=l2_loss)
        return redux_dense_layer, dense_layer

    @staticmethod
    def get_input_data(embedding_size=300, split=0):
        data = []
